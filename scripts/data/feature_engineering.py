...
class Feature_engineering_v1():
    ...
    def __init__(self, field_data, normalize):
        ...
        self.res_encoder, self.res_decoder = _result_encoding(data['result-WDL'])

        if (self.normalize):
            self.goal_scaler = _norm_scaler(data['goal_scored'], one_feature=True)
    ...
    def transforms(self, field_data, train):
        ...
        data['result-WDL'] = data['result-WDL'].apply(_encoding, args=(self.res_encoder,))
...
#         self.normalize = normalize
#
#         home_data = input_data['home'].copy(deep=True)
#         away_data = input_data['away'].copy(deep=True)
#
#         # Encoder/Decoder for TEAM, OPPONENT, F-OPPONENT
...
#
#
#     def tranforms(self, input_data, production=False):
#
...
#             home_data[col] = home_data[col].apply(_encoding, args=(self.res_encoder,))
#
#             away_data[col] = away_data[col].fillna('None')
#             away_data[col] = away_data[col].apply(_encoding, args=(self.res_encoder,))
#
#         # Normalization
#         if(self.normalize):
#
